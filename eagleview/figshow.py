...
class ImageMatrix:
    ...
    def display_image(
        ...
        num_rows = (
            math.ceil(num_images / num_columns)
            if print_all
            else self.grid_dimensions[0]
        )
        ...
        fig, axs = plt.subplots(
            num_rows, num_columns, figsize=(reduced_width, reduced_height)
        )
...
class MultiImageMatrix:
    def __init__(
        self, folder_paths=None, file_paths=None, figsize=(10, 10), axis="off"
    ):
        ...
        if len(self.folder_paths) != len(self.file_paths):
            raise ValueError(
                "Number of folder paths and file paths should be the same."
            )
    ...
    def display_image(self, *args, **kwargs):
        if len(args) > len(self.folder_paths):
            raise ValueError(
                "Number of arguments should not exceed the number of folders."
            )
        ...
        for i, im_matrix in enumerate(self.image_matrices):
            ...
            num_images = (
                im_matrix["grid_dimensions"][0] * im_matrix["grid_dimensions"][1]
                if kwargs.get("print_all", True)
                else len(images)
            )
            ...
            fig, axs = plt.subplots(
                im_matrix["grid_dimensions"][0],
                im_matrix["grid_dimensions"][1],
                figsize=(reduced_width, reduced_height),
            )
            fig.patch.set_facecolor("white")
